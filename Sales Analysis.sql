...
select Order_date, Category, Country from Chandu.sales 
	having category = 'Technology'
	and Country = 'Ireland'
	and order_date > '2020-01-01';
    
-- Q3 List the top 10 most profitable sales transactions in descending order.
	Select *,(Unit_profit * sold_quantity) as Total_Profit from chandu.sales 
		order by Total_Profit desc
		limit 10;
        
 -- Q4: Find all customers whose name starts with 'J' and ends with 'n'. 
	Select Customer_name, Country from chandu.sales where Customer_Name like 'j%n';
 
 --  Q5: Retrieve all product names that contain 'Acco' anywhere in the name.
 Select distinct product_name from chandu.sales 
	where product_name like '%acco%';
    
-- Q6: Get the top 5 cities with the highest total sales amount.
	Select City, Sum(Total_Amount) as Total_Sales from chandu.sales
     group by city
     order by Total_Sales Desc
     limit 5;
     
-- Q7. Find the total revenue, average unit cost, and total number of orders.
	Select sum(total_Amount) as Total_Revenue, avg(Unit_Cost) As Avg_Unit_Cost, count(Distinct ï»¿Order_ID) as Total_Orders from chandu.sales;  

-- Q8. Get total sales per product category.
	select category, sum(Total_Amount) as Total_Sales from chandu.sales 
		group by category 
        order by total_Sales DESC;

-- Q9: Get customers who have placed orders worth more than ₹50,000 (aggregate per customer).
	Select Customer_Name, Sum(Total_Amount) as Total_Expenses from chandu.sales
